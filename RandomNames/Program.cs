...
namespace RandomNames
{
    class Program
    {
        private static List<string> _firstNames = new List<string>() { "admiring",
                "adoring",
                "affectionate",
                "agitated",
                "amazing",
        ...
        private static string _URL = "http://localhost:20002/api/hello-world";
        private static HttpClient _client = new HttpClient();

        static void Main(string[] args)
        {
            Console.WriteLine("Posting Names");
        ...
        static async Task PostNames()
        {
            var response = await _client.PostAsync(_URL, new StringContent("{ 'who': '" + GetName() + "' }", Encoding.UTF8, "application/json"));
        }

        static string GetName()
        {
            return _firstNames[_randomizer.Next(_firstNames.Count)] + " " + _lastNames[_randomizer.Next(_lastNames.Count)];
        }
    }
}
